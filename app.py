...
def train_models():
    ...
    heart_X_train, heart_X_test, heart_y_train, heart_y_test = train_test_split(
        heart_X, heart_y, test_size=0.2, random_state=42
    )
    ...
    return (
        ...
        diabetes_model,
        diabetes_acc,
        heart_df.drop('target', axis=1).columns.tolist(),
        diabetes_df.drop('diabetes', axis=1).columns.tolist(),
    )
...
with col1:
    ...
    heart_input = pd.DataFrame([[age, sex_val, cp, trestbps, chol, fbs_val, restecg, thalach, exang_val, oldpeak, slope, ca, thal]], columns=heart_features)
...
with col2:
    ...
    diabetes_input = pd.DataFrame([[bmi, glucose, d_age, insulin, bp]], columns=diabetes_features)
...
try:
    ...
    if history:
        cols = [
            "ID", "Timestamp", "Heart Age", "Heart Sex", "Heart CP", "Heart Trestbps", "Heart Chol", "Heart FBS",
